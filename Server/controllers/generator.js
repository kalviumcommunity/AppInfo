...
function Permissions(applicationName) {
  return new Promise((resolve, reject) => {
    exec(`cd resources && ./aapt2 dump permissions ${applicationName}`, (error, stdout, stderr) => {
      if (error) {
        reject("new",error);
      } else {
...
// function funcPermissions(applicationName) {
//   let values = execSync(
//     `cd resources && ./aapt2 dump permissions ${applicationName}`
//   );

//   let Permission_s = values.toString();
...
function funcSignature(applicationName) {
  let values = execSync(
    `cd resources && keytool -printcert -jarfile ${applicationName}`
  );

  let Signature_s = values.toString();
  return Signature_s;
}
